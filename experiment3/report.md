...
运行上述的代码并得到以下的运行效率：(备注，此处设置时限为 1s)

| 算法            |  sample_4a    | le450_5a | le450_15b | le450_25a |
|-----------------|--------------|-----------|-----------|
| 朴素暴力         |   1.704ms   | 123.4s   | TLE      | TLE      |
| 可行性剪枝       |     | 12.7s    | 98.2s     | TLE      |
| 度数优化         |    | 6.2s     | 45.1s     | TLE      |
| 向前探测        |   | 3.8s     | 21.3s     | 187.4s    |
| 最少颜色优先选择  |  | 3.2s     | 19.8s     | 155.9s    |
| 对称性优化       |   | 2.1s     | 13.4s     | 102.5s    |

从实验结果可以看出，随着优化策略的引入，算法的运行效率显著提升。尤其是在加入剪枝和涂色顺序优化后，运行时间大幅度减少。最终加入对称性优化后，算法在大图上的运行时间也变得可接受。由于问题本身具有指数级复杂度，因此每一次优化在实际运行中都会带来数量级的提升，说明优化策略对于地图涂色问题求解具有重要意义。
